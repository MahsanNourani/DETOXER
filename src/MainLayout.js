...
import { Drawer as MUIDrawer, Typography, AppBar, Toolbar, IconButton, Grid, Button, ListItem, ListItemText, ListItemIcon } from '@material-ui/core';
...
export default function MainLayout() {
	...
	function submit() { // This function creates and downloads the logs as a json file
			...
			<AppBar position="fixed" className={clsx(classes.appBar, {[classes.appBarShift]: open,})}>
				<Toolbar variant="dense" classes={{root: classes.toolbar}}>
					<IconButton aria-label="open drawer" onClick={drawerToggle} className={classes.menuButton}>
						<QueuePlayNextIcon className={clsx(!open && classes.open, open && classes.close)}/>
					</IconButton>
					<Typography variant="h6" noWrap>
						Global Visualization tool
          			</Typography>
					<div className={classes.navbarButtons}>
						<Button aria-label="open global info" className={classes.navbarButton}
								startIcon={<LaunchIcon className={classes.flip}/>} onClick={globalDialogeToggle}>
							Global Information
						</Button>
						<Button aria-label="open notepad" className={classes.navbarButton}
								startIcon={<MenuBookIcon/>} onClick={notebookToggle}>
							Notepad
						</Button>
						<Button aria-label="submit" className={classes.navbarButton}
								startIcon={<AssignmentTurnedInIcon/>} onClick={submit} disabled= {!submitEnabled}>
							Submit
						</Button>
					</div>
				...
				<div className={classes.drawerHeader}>
					 <Grid container spacing={1}>
						...
						<Grid container item md={4} spacing={1}>
							<Grid item md={12}>
								<GlobalInfoCard open={openGlobalInfo} close={globalDialogeClose} />
							</Grid>
							<Grid item md={12}>
