...
export const ProgressBar = (props) => {
  ...
  const changeMovingBar = (t, id, parent) => {
    ...
    const svgWidth =
      document.getElementsByClassName(parent)[0].clientWidth + 0.4;
  ...
  const handleSeekMouseUp = (e) => {
    ...
    createTimeLog(
      ...
      "slider",
      parseFloat(
        parseFloat(parseFloat(e.target.value) * videoDuration).toFixed(3)
      )
    );
  };
  return (
    <input
      ...
      className={`${classes.slider} slider`}
    />
  );
};
...
export const PlayerControls = (props) => {
      ...
      const tempHeatmap = data.map((item, index) => {
        ...
        return (
          <Heatmap
            ...
            type={item.Type}
            order={item.Order}
            change={handleSeekHeatmapChange}
          />
        );
      });
  ...
  function updateStreams() {
    if (data && data.length) {
      let key =
        data[1].componentName + "-" + data[0].Frame1 + "-" + data[1].Frame1;
      const tempStreamgraph = (
        <Streamgraph
          size={[250, 120]}
          streamData={data}
          key={key}
  ...
  const legend = legendData.map((item, index) => {
    return (
      ...
      >
        ...
        <Typography variant="caption">[{item.prob}]</Typography>
      </Grid>
    );
  });
  ...
  const viewSelect = (
    <FormControl className={classes.formControl}>
      <NativeSelect
        ...
        className={classes.sortAndFilterSelect}
        inputProps={{ name: "view", id: "view" }}
      >
  ...
  return (
    <React.Fragment>
      ...
      {/* Uncomment the following for different views */}
      {/* <Grid container>
        <Grid item>{viewSelect}</Grid>
      </Grid> */}
      <Grid
        container
        alignItems="center"
        ...
        className={clsx(
          ...
          viewValue == "streamgraph" ? classes.invisible : ""
        )}
      >
        ...
        <div
          className={clsx(
            ...
            document.getElementsByTagName("main")[0] &&
              document.getElementsByTagName("main")[0].clientWidth > 1100
              ? classes.slideBarTall
              : classes.sliderBarShort
          )}
          id="movingBarHeatmap"
        ></div>
      ...
      <Grid
        ...
        justify="center"
        className={clsx(
          `${classes.streamgraphContainer} streamgraphMainDiv`,
          viewValue == "heatmap"
            ? classes.invisible
            : viewValue == undefined
            ? classes.invisible
      ...
      >
        <div
          className={clsx(
            classes.sliderBarStreamgraph,
            document.getElementsByTagName("main")[0] &&
              document.getElementsByTagName("main")[0].clientWidth > 1100
              ? classes.slideBarTall
              : classes.sliderBarShort
          )}
          id="movingBarStreamgraph"
        ></div>
        {streamgraphs}
      </Grid>
    </React.Fragment>
